...
export const api = {
  ...
  supplementTree: (params = {}) => request(`/api/v2/supplement/tree?${new URLSearchParams(params).toString()}`),
  supplementResolveFsp: (payload) => request("/api/v2/supplement/resolve-fsp", { method: "POST", body: JSON.stringify(payload) }),
  supplementOpenFsp: (payload) => request("/api/v2/supplement/open-fsp", { method: "POST", body: JSON.stringify(payload) }),
  supplementMarkFspStatus: (payload) => request("/api/v2/supplement/mark-fsp-status", { method: "POST", body: JSON.stringify(payload) }),
  supplementInheritedParams: (params = {}) => request(`/api/v2/supplement/inherited-params?${new URLSearchParams(params).toString()}`),
  supplementPreviewPlan: (payload) => request("/api/v2/supplement/preview-plan", { method: "POST", body: JSON.stringify(payload) }),
  supplementRun: (payload) => request("/api/supplement/run", { method: "POST", body: JSON.stringify(payload) }),
  supplementJobState: (jobId) => request(`/api/supplement/jobs/${encodeURIComponent(jobId)}`),
  supplementJobEvents: (jobId, params = {}) => request(`/api/supplement/jobs/${encodeURIComponent(jobId)}/events?${new URLSearchParams(params).toString()}`),
  supplementOpenFolder: (payload) => request("/api/supplement/open-folder", { method: "POST", body: JSON.stringify(payload) }),
